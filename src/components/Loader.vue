<template>
  <Dialog :showDialog="appLoading">
    <Card v-if="appLoading">
      <template #content>
        <svg
          width="190"
          height="40"
          viewBox="0 0 190 19"
          version="1.1"
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
        >
          <defs>
            <path id="path-1" d="M0 .03h7.737v18.604H0z" />
            <path id="path-3" d="M.135.115H6.6v6.464H.135z" />
          </defs>
          <g
            id="Page-1"
            stroke="none"
            stroke-width="1"
            fill="none"
            fill-rule="evenodd"
          >
            <g id="main-menu" transform="translate(-108 -30)">
              <g id="header">
                <g id="Group-21" transform="translate(108 30)">
                  <g id="Group-3" transform="translate(0 .153)">
                    <mask id="mask-2" fill="#fff">
                      <use xlink:href="#path-1" />
                    </mask>
                    <path
                      d="M6.06 6.974c1.066 0 1.677-.58 1.677-1.593 0-.954-.657-1.594-1.635-1.594H3.685v-1.79C3.685.706 2.758.031 1.842.031.927.03 0 .706 0 1.997v12.346c0 2.967 1.183 4.29 3.834 4.29 1.437 0 2.259-.369 2.869-.728.467-.26.633-.81.633-1.238 0-.794-.53-1.679-1.293-1.679a1.014 1.014 0 00-.461.116c-.2.092-.475.218-.877.218-.475 0-1.02-.168-1.02-1.477v-6.87h2.376z"
                      id="Fill-1"
                      fill="#1D1C39"
                      mask="url(#mask-2)"
                      class="animation t"
                    />
                  </g>
                  <path
                    d="M14.582 6.628c.466-.17.93-.256 1.378-.256 1.533 0 2.605.99 3.136 1.977l-6.866 2.499c-.314-1.484.118-3.407 2.352-4.22m7.374 6.854a1.68 1.68 0 00-1.044.386l-.052.046a6.841 6.841 0 01-2.26 1.327 5.596 5.596 0 01-1.909.36c-1.592 0-2.93-.851-3.617-2.287l8.471-3.083c1.334-.486 1.513-1.472 1.236-2.234-1.034-2.84-3.47-4.605-6.358-4.605-.942 0-1.921.182-2.91.542-2.172.79-3.718 2.216-4.474 4.122-.684 1.728-.667 3.76.046 5.72 1.155 3.174 3.612 4.994 6.741 4.994a9.11 9.11 0 001.843-.197 11.531 11.531 0 001.624-.458c1.417-.515 2.708-1.26 3.732-2.153l.01-.008.008-.009c.34-.385.475-.844.38-1.292-.14-.668-.77-1.171-1.467-1.171"
                    id="Fill-4"
                    fill="#1D1C39"
                    class="animation e"
                  />
                  <path
                    d="M36.172 11.135c0 2.16-1.227 4.34-3.967 4.34-2.348 0-3.925-1.727-3.925-4.299 0-2.62 1.594-4.38 3.966-4.38 1.954 0 3.926 1.341 3.926 4.34m1.925-7.652c-1.092 0-1.884.775-1.884 1.842v.059c-1.085-1.23-2.693-1.901-4.589-1.901-4.279 0-7.153 3.075-7.153 7.651 0 4.577 2.824 7.652 7.029 7.652 2.06 0 3.74-.749 4.796-2.122v.28c0 1.358.93 1.842 1.8 1.842 1.086 0 1.76-.706 1.76-1.843V5.326c0-1.119-.69-1.842-1.76-1.842"
                    id="Fill-6"
                    fill="#1D1C39"
                    class="animation a"
                  />
                  <path
                    d="M59.272 3.484c-2.164 0-3.838.958-4.654 2.645-.896-1.733-2.506-2.645-4.682-2.645-1.62 0-3.069.642-3.926 1.707-.026-.993-.792-1.707-1.842-1.707-.937 0-1.884.595-1.884 1.925v11.535c0 1.189.655 1.843 1.843 1.843 1.18 0 1.884-.689 1.884-1.843v-6.888c0-2.274 1.013-3.427 3.012-3.427 1.963 0 2.764 1.09 2.764 3.76v6.514c0 1.215.654 1.884 1.842 1.884 1.197 0 1.883-.687 1.883-1.884V10.47c0-2.585 1-3.842 3.055-3.842 1.937 0 2.763 1.15 2.763 3.842v6.432c0 1.215.654 1.884 1.842 1.884 1.198 0 1.884-.687 1.884-1.884V10.47c0-4.636-1.946-6.987-5.784-6.987"
                    id="Fill-8"
                    fill="#1D1C39"
                    class="animation m"
                  />
                  <path
                    d="M87.543 3.484c-.819 0-1.504.59-1.832 1.574l-3.146 8.889L79.458 5.1c-.337-.97-.763-1.616-1.749-1.616-1.053 0-1.442.569-1.79 1.616l-3.107 8.847-3.145-8.886c-.329-.987-1.014-1.577-1.833-1.577-.783 0-1.594.643-1.594 1.718 0 .344.08.687.263 1.145l4.232 10.79c.405.99.913 1.65 2.078 1.65 1.275 0 1.76-.885 2.039-1.656l2.855-7.782 2.775 7.777c.367 1.102 1.068 1.66 2.082 1.66.968 0 1.681-.554 2.12-1.652l4.187-10.82c.182-.408.266-.761.266-1.112 0-.98-.685-1.718-1.594-1.718"
                    id="Fill-10"
                    fill="#1D1C39"
                    class="animation w"
                  />
                  <path
                    d="M97.018 15.848c-2.447 0-3.967-1.806-3.967-4.713 0-2.813 1.557-4.63 3.967-4.63s3.967 1.817 3.967 4.63c0 2.863-1.557 4.713-3.967 4.713m0-12.323c-2.417 0-4.418.794-5.789 2.295-1.227 1.346-1.904 3.233-1.904 5.315 0 2.085.676 3.98 1.904 5.335 1.372 1.516 3.374 2.317 5.79 2.317 5.677 0 7.692-4.122 7.692-7.652 0-3.51-2.015-7.61-7.693-7.61"
                    id="Fill-12"
                    fill="#1D1C39"
                    class="animation o"
                  />
                  <path
                    d="M113.711 3.484a4.881 4.881 0 00-3.723 1.694 1.845 1.845 0 00-1.836-1.694c-1.033 0-1.843.81-1.843 1.842v11.618c0 1.21.927 1.843 1.843 1.843.915 0 1.842-.633 1.842-1.843v-5.519c0-3.332.913-4.754 3.054-4.754.341 0 .539.045.73.09.169.039.328.076.556.076.879 0 1.469-.657 1.469-1.635 0-1.044-.821-1.718-2.092-1.718"
                    id="Fill-14"
                    fill="#1D1C39"
                    class="animation r"
                  />
                  <path
                    d="M124.3 11.174l4.024-4.186c.56-.56 1.089-1.09 1.089-1.828 0-.955-.721-1.676-1.677-1.676-.749 0-1.28.574-1.793 1.13l-4.87 5.032v-7.62A1.844 1.844 0 00119.23.182a1.845 1.845 0 00-1.843 1.842v14.92c0 1.015.826 1.841 1.843 1.841a1.844 1.844 0 001.842-1.842V12.42l4.874 5.069c.086.086.17.174.254.261.489.51.994 1.037 1.743 1.037.886 0 1.635-.748 1.635-1.634 0-.644-.437-1.135-.884-1.582l-4.396-4.396z"
                    id="Fill-16"
                    fill="#1D1C39"
                    class="animation k"
                  />
                  <g id="Group-20" transform="translate(131.582 12.215)">
                    <mask id="mask-4" fill="#fff">
                      <use xlink:href="#path-3" />
                    </mask>
                    <path
                      d="M3.367.115A3.236 3.236 0 00.135 3.347a3.236 3.236 0 003.232 3.232A3.236 3.236 0 006.6 3.347 3.236 3.236 0 003.367.115"
                      id="Fill-18"
                      class="animation"
                      fill="#FF22B1"
                      mask="url(#mask-4)"
                    />
                  </g>

                  <g id="Group-22" transform="translate(139.582 12.215)">
                    <mask id="mask-5" fill="#fff">
                      <use xlink:href="#path-3" />
                    </mask>
                    <path
                      d="M3.367.115A3.236 3.236 0 00.135 3.347a3.236 3.236 0 003.232 3.232A3.236 3.236 0 006.6 3.347 3.236 3.236 0 003.367.115"
                      id="Fill-19"
                      fill="#FF22B1"
                      class="animation"
                      mask="url(#mask-5)"
                    />
                  </g>
                  <g id="Group-23" transform="translate(147.582 12.215)">
                    <mask id="mask-6" fill="#fff">
                      <use xlink:href="#path-3" />
                    </mask>
                    <path
                      d="M3.367.115A3.236 3.236 0 00.135 3.347a3.236 3.236 0 003.232 3.232A3.236 3.236 0 006.6 3.347 3.236 3.236 0 003.367.115"
                      id="Fill-20"
                      fill="#FF22B1"
                      class="animation"
                      mask="url(#mask-6)"
                    />
                  </g>
                </g>
              </g>
            </g>
          </g>
        </svg>
      </template>
    </Card>
  </Dialog>
</template>

<script>
import { mapState } from "vuex";
import Card from "@/components/Card.vue";
import Dialog from "@/components/Dialog.vue";
export default {
  name: "Loader",
  components: {
    Card,
    Dialog
  },
  computed: mapState(["appLoading"])
};
</script>

<style>
.animation {
  animation: jump 1s linear infinite;
}

.e {
  animation-delay: 0.2s;
}
.a {
  animation-delay: 0.3s;
}
.m {
  animation-delay: 0.4s;
}
.w {
  animation-delay: 0.5s;
}
.o {
  animation-delay: 0.6s;
}
.r {
  animation-delay: 0.7s;
}
.k {
  animation-delay: 0.8s;
}

#Fill-18 {
  animation-delay: 0.854s;
}
#Fill-19 {
  animation-delay: 0.89s;
}
#Fill-20 {
  animation-delay: 1s;
}
@keyframes jump {
  0%,
  20%,
  100% {
    transform: translateY(0);
  }
  13% {
    transform: translateY(-10px);
  }
}
</style>
